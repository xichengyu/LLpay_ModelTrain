...
def normalize_input_data(json_fmt, keys, bounds, merge_bounds, dum_coding_fields):
    ...
    try:
        ...
        for key in origin_keys:
            if key in bounds:
                origin_keys[key] = [abs(x - bounds[key][0]) / (bounds[key][1] - bounds[key][0]) for x in
                                    origin_keys[key]]
        print("max_min:\n", len(origin_keys), len(origin_keys['unixTime']))
...
if __name__ == '__main__':
    ...
    raw_data = lr.load_df_data(conf_info["raw_data"])  # get original data
    ...
    try:
        ...
        for strategy in strategies:
            new_data = mvs.fill_strategy(raw_data, strategy, missing_values)
            ...
            for i in range(run_times):
                ...
                for train_data in train_data_list:
                    ...
                    if 0:
                        ModelTrain.train_model(train_data, train_target, dum_coding_fields, algorithm,
                                               preprocessing_flag)
