...
data_src = 'local'
delim = ";"
# target_fields = ['clicked', 'reqid', 'mediasi', 'fcsid', 'price', 'ip', 'lng', 'lat', 'timestamp', 'network', 'co', 'app'
#                 , 'app_version', 'deviceid', 'mac', 'os', 'os_version', 'model', 'text_id']

target_fields = ['clicked', 'mediasi', 'fcsid', 'timestamp', 'network', 'co', 'os', 'text_id']

dum_coding_fields = ['mediasi', 'fcsid', 'timestamp', 'network', 'co', 'os', 'text_id']
...
def normalize_input_data(json_fmt, keys, bounds, merge_bounds, dum_coding_fields):
    ...
    try:
        ...
        origin_len = len(origin_keys['timestamp'])
        ...
        # print "merge:\n", len(origin_keys.columns), origin_len)

        '''max_min merged results'''
        # print keys
        # print origin_keys.columns
        for key in keys:
            dic[key] = [0] * origin_len
            if key in origin_keys.columns:
                dic[key] = origin_keys[key]
        # print DataFrame(dic)
    except:
        traceback.print_exc()
        pass
...
if __name__ == '__main__':
    ...
    data_path = "../../data/fuchunjiang.csv"

    algorithm = "RF"      # RF, GBDT, LR

    total_partition_n = [10.0]
    train_partition_n = [2.0]
    ...
    try:
        for k, v in dict(zip(train_partition_n, total_partition_n)).items():
            ...
            for train_data in train_data_list:
                ...
                if 1:
                    FcjTrain.train_model(train_data, dum_coding_fields, algorithm, preprocessing_flag)
